...
from data.ava import HEIGHT, WIDTH, AVADataset, detection_collate
...
from utils.tube_utils import compute_box_iou, valid_tubes
...
def main():
    ...
    if args.num_classes == 60:
        label_map = os.path.join(
            args.data_root, "label/ava_action_list_v2.1_for_activitynet_2018.pbtxt"
        )
    ...
    dataset = AVADataset(
        ...
        proposal_path=args.proposal_path_val,
        stride=1,
    ...
    for i in range(args.max_iter):
        ...
        metrics = ava_evaluation(
            os.path.join(args.data_root, "label/"), output_files[i], gt_file
        )
    ...
    for i in range(args.max_iter):
        prt_str += (
            ...
            + str(i + 1)
            + ": MEANAP =>"
            + str(all_metrics[i]["PascalBoxes_Precision/mAP@0.5IOU"])
            + "\n"
        )
    ...
    for i in class_whitelist:
        log_file.write(
            "({}) {}: {}\n".format(
                ...
                id2class[i],
                all_metrics[-1][
                    "PascalBoxes_PerformanceByCategory/AP@0.5IOU/{}".format(id2class[i])
                ],
            )
        )
