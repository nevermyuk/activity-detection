...
videodir = "D:/SIT/Year3-1/ICT3104/Project/nvda-ml-activity-detection/step/datasets/ava/videos/"  # TODO: put the path to your AVA dataset here
...
for video_id in videonames:
    ...
    for mid in range(900, 1800):
        ...
        if not os.path.isdir(os.path.join(clips_dir, "{:05d}".format(mid))):
            ...
            ffmpeg_command = "ffmpeg -ss {} -i {} -qscale:v 4 -vf scale=-1:360 -t {} {}/%05d.jpg".format(
                mid,
                videofile,
                clip_length,
                os.path.join(clips_dir, "{:05d}".format(mid)),
            )
