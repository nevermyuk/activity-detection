...
from data.customize import HEIGHT, WIDTH, CustomizedDataset, detection_collate
from external.maskrcnn_benchmark.roi_layers import nms
from models import BaseNet, ContextNet, ROINet, TwoBranchNet
from utils.tube_utils import compute_box_iou, flatten_tubes, valid_tubes
from utils.utils import AverageMeter, get_gpu_memory, inference, train_select
from utils.vis_utils import overlay_image


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("-data_root", type=str, default="False")
...
def main():
    ...
    print(custom_args.data_root)
    args.data_root = custom_args.data_root
    ...
    t0 = time.time()
    with torch.no_grad():
        for _, (images, tubes, infos) in enumerate(
            tqdm(dataloader, desc="Inferencing...")
        ):
            ...
            history, _ = inference(
                args, conv_feat, context_feat, nets, args.max_iter, tubes
            )
            ...
            for b in range(len(tubes_nums)):
                ...
                for i in range(len(scores_list)):
                    if flag[i]:
                        ...
                        merged_box = np.mean(
                            np.concatenate(temp[0], axis=0).reshape(-1, 4), axis=0
                        )
                ...
                overlay_image(
                    os.path.join(
                        args.data_root, info["video_name"], im_format % info["fid"]
                    ),
                    os.path.join(
                        args.save_root, info["video_name"], im_format % info["fid"]
                    ),
